...
object Example_Rdd_map_180420 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //Map 함수 적용 (Row 있을 때)
    // filteredRdd.first
    // filteredRdd = (key, account, product, yearweek, qty, productname)
    // 처리 로직: 거래량이 MAXVALUE 이상인 건은 MAXVALUE로 치환한다

    var MAXVALUE = 700000  // 원래 전역변수는 맨 위에서 선언해야 함
